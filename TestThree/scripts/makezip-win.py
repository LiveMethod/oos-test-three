...
def main():
  ...
  if not zip:
    installer = "\\build-win\\installer\\TestThree Installer.exe"
    
    if demo:
      installer = "\\build-win\\installer\\TestThree Demo Installer.exe"
    
    files = [
      projectpath + installer,
      projectpath + "\\installer\\changelog.txt",
      projectpath + "\\installer\\known-issues.txt",
      projectpath + "\\manual\\TestThree manual.pdf" 
    ]
  else:
    files = [
      projectpath + "\\build-win\\TestThree.vst3\\Contents\\x86_64-win\\TestThree.vst3",
      projectpath + "\\build-win\\TestThree_x64.exe"  
    ]
  ...
  files = [
    projectpath + "\\build-win\\pdbs\\TestThree-vst3_x64.pdb",
    projectpath + "\\build-win\\pdbs\\TestThree-app_x64.pdb"  
  ]
