...
# WIN INSTALLER
  ...
  for line in fileinput.input(projectpath + "/installer/" + config['BUNDLE_NAME'] + ".iss",inplace=1):
    ...
    if "OutputBaseFilename" in line:
      if demo:
        line="OutputBaseFilename=TestThree Demo Installer\n"
      else:
        line="OutputBaseFilename=TestThree Installer\n"
    ...
    if "WelcomeLabel1" in line:
     if demo:
       line="WelcomeLabel1=Welcome to the TestThree Demo installer\n"
     else:
       line="WelcomeLabel1=Welcome to the TestThree installer\n"
       
    if "SetupWindowTitle" in line:
     if demo:
       line="SetupWindowTitle=TestThree Demo installer\n"
     else:
       line="SetupWindowTitle=TestThree installer\n"
